...
Outputs:
  - SUCCESS manifest (CSV): AttachmentId, ParentId, FileName, ContentType, BodyLength,
    CreatedDate, LastModifiedDate, SystemModstamp, S3RelativePath, S3AbsolutePath,
    Status, Timestamp
  ...
  - BATCH metrics (JSON, one object per Spark partition): written to
    <BATCH_METRICS_OUTPUT_PATH>/<job_name>/partition_<NNNNN>_<batch_start_ts>.json
    as each partition finishes, with per-partition counts (rows seen,
    successes, failures broken down by ErrorType, average attempts, whether
    the org's daily API limit was hit, batch start/end/duration).
  - JOB metrics (single JSON): written to
    <JOB_METRICS_OUTPUT_PATH>/<job_name>_<job_end_ts>.json once the whole run
    completes, aggregating counts across all partitions (total rows, success/
    failed counts, corrupt-row count, failed-by-error-type breakdown, job
    duration, and the resolved job parameters used for the run).
  - "job_name" for both metrics folders is derived from the input CSV's
    filename (path stripped, extension stripped) so metrics from different
    input batches/files land in clearly separated, identifiable locations
    instead of all mixing together under one prefix.
...
Edge cases explicitly handled:
  ...
  - Empty input file / empty partitions short-circuit without wasting a
    Salesforce login call (and, for empty partitions, without writing a
    meaningless zero-row batch metrics file).
  - A Salesforce login failure at the start of a partition (bad creds, network
    ...
    A batch metrics file is still written for that partition (best-effort) so
    the failure is visible in the metrics without digging through task logs.
  ...
  - Salesforce's *daily* API limit (403 REQUEST_LIMIT_EXCEEDED) is treated
    ...
    This flag is also recorded in that partition's batch metrics.
  ...
  - Batch metrics writes are best-effort: a failure to write a partition's
    metrics JSON (e.g. transient S3 error) is logged and swallowed, never
    allowed to fail the Spark task -- metrics are diagnostic, not part of the
    job's correctness contract.
...
Designed for 4M+ rows:
  ...
  - boto3 S3 client created once per partition (boto3 clients are not
    ...
    stalled connection can't hang a partition indefinitely. The same client is
    reused to write that partition's batch metrics JSON at the end.
  ...
  - Job-level metrics reuse success_df/failed_df.count() and a cheap
    failed_df.groupBy("ErrorType").count() aggregation -- both operate on
    result_df, which is already persisted after the download/upload pass, so
    computing metrics doesn't re-trigger any Salesforce downloads or S3
    uploads.
...
Required Glue job parameters (--arg-name value):
  ...
  --BATCH_METRICS_OUTPUT_PATH  s3://your-output-bucket/metric/batch/
  --JOB_METRICS_OUTPUT_PATH    s3://your-output-bucket/metric/job/
...
Recommended Glue/Spark setting for this job:
  ...
  job's tasks perform real side effects (S3 uploads + manifest rows + batch
  metrics writes) rather than pure functions, a speculative re-run risks
  duplicate manifest rows / duplicate batch metrics files for the same
  partition. Turning speculation off avoids that; it's not set inside this
  script because it's a cluster-level setting, not a job argument.
...
API, not carried in the CSV):
  ...
  - "LastModifiedDate" and "SystemModstamp" are carried straight through
    (no parsing) into both the SUCCESS and FAILED output manifests, so
    downstream consumers can tell how fresh/stale a given attachment record
    was in Salesforce at extract time without re-querying Salesforce.
  - Only Id, ParentId, Name, ContentType, BodyLength, CreatedDate,
    LastModifiedDate, and SystemModstamp are actually used for processing;
    the remaining columns are accepted so the CSV can be a straight,
    unmodified extract of the Attachment table, but are dropped before
    processing (see clean_df.select(...) in main()) to keep the
    per-partition shuffle payload small.
"""
...
ARGS = getResolvedOptions(
    ...
    [
        ...
        "SUCCESS_OUTPUT_PATH",
        "FAILED_OUTPUT_PATH",
        "BATCH_METRICS_OUTPUT_PATH",
        "JOB_METRICS_OUTPUT_PATH",
        "SF_SECRET_NAME",
        "SF_API_VERSION",
        "SF_LOGIN_URL",
        "NUM_PARTITIONS",
...
BATCH_METRICS_OUTPUT_PATH = ARGS["BATCH_METRICS_OUTPUT_PATH"]
JOB_METRICS_OUTPUT_PATH = ARGS["JOB_METRICS_OUTPUT_PATH"]
...
# Module-level logger used INSIDE partition functions. glueContext's `logger`
# is a driver-side object and is deliberately never referenced from within
# process_partition/process_row -- pulling it into that closure would drag a
# non-picklable object across the driver->executor boundary. This plain
# stdlib logger is safe to use from executor threads instead.
_executor_log = logging.getLogger("sf_attachment_downloader")

# --------------------------------------------------------------------------- #
# Job/batch naming + metrics path helpers
# --------------------------------------------------------------------------- #


def derive_job_name(input_path: str) -> str:
    """Derive a stable, filesystem/S3-key-safe identifier for this run from
    the input CSV's filename, e.g. s3://bucket/input/attachment_replica.csv
    -> "attachment_replica". Used to namespace both the batch and job metrics
    so multiple input files' metrics never mix together under one prefix,
    and so a retry pass (fed a FAILED manifest as its new input) gets its own
    clearly distinct metrics trail."""
...
def parse_s3_path(s3_path: str):
    """Split an s3://bucket/some/prefix/ URL into (bucket, prefix)."""
...
BATCH_JOB_NAME = derive_job_name(INPUT_CSV_PATH)
BATCH_METRICS_BUCKET, BATCH_METRICS_PREFIX = parse_s3_path(BATCH_METRICS_OUTPUT_PATH)
JOB_METRICS_BUCKET, JOB_METRICS_PREFIX = parse_s3_path(JOB_METRICS_OUTPUT_PATH)

# Broadcast the (small, static) info each partition needs to name and write
# its own batch metrics file -- avoids re-deriving/re-parsing this on every
# partition and keeps the closure passed to mapPartitionsWithIndex small.
BATCH_METRICS_INFO_BC = sc.broadcast({
    "bucket": BATCH_METRICS_BUCKET,
    "prefix": BATCH_METRICS_PREFIX,
...
# --------------------------------------------------------------------------- #
# Batch (per-partition) metrics helpers
# --------------------------------------------------------------------------- #
# "Batch" here means one Spark partition of the input -- the natural unit of
# work a single process_partition() call handles. Each partition accumulates
# its own counts locally (no cross-partition coordination needed) and writes
# one JSON metrics file describing itself once it finishes.

def _new_batch_metrics(partition_id: int) -> dict:
    return {
        "partition_id": partition_id,
        "batch_start_ts": dt.datetime.utcnow().isoformat(),
        "batch_end_ts": None,
        ...
        "avg_attempts": None,
        "success_rate": None,
        "org_api_limit_hit": False,
    }


def _record_row_in_batch_metrics(metrics: dict, result_row: Row) -> None:
    metrics["total_rows"] += 1
    if result_row["Status"] == "SUCCESS":
        metrics["success_count"] += 1
    else:
        metrics["failed_count"] += 1
        error_type = result_row["ErrorType"] or "Unknown"
        metrics["failed_by_error_type"][error_type] = metrics["failed_by_error_type"].get(error_type, 0) + 1
    metrics["total_attempts"] += result_row["Attempts"] or 0


def _finalize_and_write_batch_metrics(metrics: dict, org_limit_hit: bool,
                                       metrics_info: dict, s3_client=None,
                                       status: str = "COMPLETED",
                                       note: str = None) -> None:
    """Finalizes derived fields (duration, rates) and best-effort writes the
    partition's metrics JSON to S3. Never raises -- a metrics write failure
    must not fail the Spark task."""
    metrics["batch_end_ts"] = dt.datetime.utcnow().isoformat()
    metrics["org_api_limit_hit"] = org_limit_hit
    metrics["status"] = status
    if note:
        metrics["note"] = note
    metrics["job_name"] = metrics_info["job_name"]
    metrics["source_file"] = metrics_info["source_file"]
    try:
        start = dt.datetime.fromisoformat(metrics["batch_start_ts"])
        end = dt.datetime.fromisoformat(metrics["batch_end_ts"])
        metrics["duration_seconds"] = (end - start).total_seconds()
    except Exception:
        metrics["duration_seconds"] = None
    if metrics["total_rows"]:
        metrics["avg_attempts"] = metrics["total_attempts"] / metrics["total_rows"]
        metrics["success_rate"] = metrics["success_count"] / metrics["total_rows"]

    safe_start_ts = metrics["batch_start_ts"].replace(":", "-")
    key = (
        f'{metrics_info["prefix"]}/{metrics_info["job_name"]}/'
        f'partition_{metrics["partition_id"]:05d}_{safe_start_ts}.json'
    )
    ...
    try:
        ...
        client.put_object(
            Bucket=metrics_info["bucket"], Key=key, Body=body,
            ContentType="application/json",
        )
    except Exception as e:  # noqa: BLE001 - metrics writes are best-effort
        _executor_log.warning(
            "Failed to write batch metrics for partition %s to s3://%s/%s: %s",
            metrics["partition_id"], metrics_info["bucket"], key, e,
        )
...
RESULT_FIELDS = [
    ...
    "CreatedDate", "LastModifiedDate", "SystemModstamp", "S3RelativePath",
    "S3AbsolutePath", "Status", "ErrorType", "ErrorMessage", "Attempts",
    "Timestamp",
]


def process_partition(partition_id, rows):
    metrics_info = BATCH_METRICS_INFO_BC.value

    # Peek at the first row without fully materializing the partition, so
    # empty partitions skip all setup entirely (no wasted API calls, and no
    # meaningless zero-row batch metrics file).
    row_iter = iter(rows)
    try:
        first_row = next(row_iter)
    except StopIteration:
        return  # empty partition -- nothing to do, nothing to log
    row_iter = itertools.chain([first_row], row_iter)

    batch_metrics = _new_batch_metrics(partition_id)
    ...
    try:
        ...
        try:
            fallback_s3_client = boto3.client("s3")
        except Exception:  # noqa: BLE001 - metrics best-effort even here
            pass
        for row in row_iter:
            result_row = Row(
                ...
                LastModifiedDate=row["LastModifiedDate"], SystemModstamp=row["SystemModstamp"],
            ...
            _record_row_in_batch_metrics(batch_metrics, result_row)
            yield result_row
        _finalize_and_write_batch_metrics(
            batch_metrics, org_limit_hit=False, metrics_info=metrics_info,
            s3_client=fallback_s3_client, status="SETUP_FAILED", note=err_msg,
        )
        return
    ...
    def process_row(row) -> Row:
        ...
        def fail(error_type, error_message, attempts=0, filename=None):
            return Row(
                ...
                SystemModstamp=system_modstamp, S3RelativePath=None, S3AbsolutePath=None,
    ...
    try:
        ...
        # Batch metrics are finalized/written here so they cover the whole
        # partition (including rows processed right up to the last future),
        # regardless of whether the loop above finished normally or the
        # generator was torn down early. Written before the session is
        # closed so s3_client (unaffected by session.close(), but kept in
        # the same scope) is still valid to use.
        _finalize_and_write_batch_metrics(
            batch_metrics, org_limit_hit=org_limit_hit.is_set(),
            metrics_info=metrics_info, s3_client=s3_client, status="COMPLETED",
        )
        session.close()
...
def main():
    job_start_ts = dt.datetime.utcnow().isoformat()
    logger.info(f"Reading input CSV from {INPUT_CSV_PATH} (job_name={BATCH_JOB_NAME})")
    ...
    def write_job_metrics(extra: dict):
        """Best-effort write of the single job-level metrics JSON. Never
        raises -- a metrics write failure must not fail the job after all
        the actual download/upload work has already completed."""
        job_end_ts = dt.datetime.utcnow().isoformat()
        try:
            start = dt.datetime.fromisoformat(job_start_ts)
            end = dt.datetime.fromisoformat(job_end_ts)
        ...
        payload = {
            ...
            "batch_metrics_output_path": BATCH_METRICS_OUTPUT_PATH,
        }
        payload.update(extra)

        key = f"{JOB_METRICS_PREFIX}/{BATCH_JOB_NAME}_{job_end_ts.replace(':', '-')}.json"
        try:
            client = boto3.client("s3")
            client.put_object(
                Bucket=JOB_METRICS_BUCKET, Key=key,
                Body=json.dumps(payload, default=str).encode("utf-8"),
                ContentType="application/json",
            )
            logger.info(f"Wrote job metrics to s3://{JOB_METRICS_BUCKET}/{key}")
        except Exception as e:  # noqa: BLE001 - metrics are best-effort
            logger.warn(f"Failed to write job metrics to s3://{JOB_METRICS_BUCKET}/{key}: {e}")
    ...
    if df.rdd.isEmpty():
        ...
        write_job_metrics({
            "total_rows_processed": 0, "success_count": 0, "failed_count": 0,
            "corrupt_row_count": 0, "success_rate": None,
            "failed_by_error_type": {}, "status": "EMPTY_INPUT",
        })
        job.commit()
        return
    ...
    # these eight columns are used for processing / carried through to the
    # output manifests. Selecting down before the repartition shuffle below
    # keeps the shuffle payload small instead of dragging OwnerId,
    # Description, audit ID columns, etc. through every partition boundary
    # for 4M+ rows. LastModifiedDate and SystemModstamp are kept (unlike the
    # other audit columns) because they're passed straight through into the
    # SUCCESS/FAILED manifests.
    clean_df = clean_df.select(
        "Id", "ParentId", "Name", "ContentType", "BodyLength",
    ...
    # mapPartitionsWithIndex (rather than mapPartitions) so each partition
    # knows its own index -- used purely to name/identify that partition's
    # batch metrics file; processing logic itself doesn't depend on it.
    result_rdd = clean_df.rdd.mapPartitionsWithIndex(process_partition)
    ...
    result_df.persist()  # reused for both success/failed writes and job metrics
    ...
    # and the groupBy below read from cache rather than re-triggering any
    # Salesforce downloads or S3 uploads -- cheap relative to the job's
    # actual I/O work.
    success_count = success_df.count()
    failed_count = failed_df.count()
    logger.info(f"Done. SUCCESS={success_count} FAILED={failed_count}")

    # Cheap aggregation (not a driver-side collect of raw rows) giving the
    # job-level metrics a breakdown of *why* rows failed, mirroring the
    # per-batch failed_by_error_type breakdown but rolled up for the whole run.
    failed_by_error_type = {
        r["ErrorType"] or "Unknown": r["count"]
        for r in failed_df.groupBy("ErrorType").count().collect()
    }
    ...
    write_job_metrics({
        ...
        "success_rate": (success_count / total_rows) if total_rows else None,
